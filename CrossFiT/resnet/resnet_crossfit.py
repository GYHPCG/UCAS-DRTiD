...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if self.pool == 'cat':
            self.xfmer_fc = nn.Sequential(
                nn.LayerNorm(self.xfmer_hidden_size*2),
                nn.Linear(self.xfmer_hidden_size*2, num_classes)
            )
        else:
            self.xfmer_fc = nn.Sequential(
        ...
        # self.learnable_pos_embed1 = nn.Parameter(torch.randn(1, 16*16, self.xfmer_hidden_size))
        # self.learnable_pos_embed2 = nn.Parameter(torch.randn(1, 16*16, self.xfmer_hidden_size))
    ...
    def _forward_impl(self, x1, x2, img1_grid: Tensor) -> Tensor:
    ...
    def forward(self, x1, x2, img1_grid: Tensor) -> Tensor:
        return self._forward_impl(x1, x2, img1_grid)
...
class Attention(nn.Module):
    ...
    def forward(self, query, key=None, *, value=None, mask=None):
        ...
        if mask is not None:
            ...
            mask = mask.data.masked_fill_(mask.byte(),-float('inf'))
...
class Transformer_head(nn.Module):
    def __init__(self, hidden_size=1024, layers=3):
        super().__init__()
        blocks = []
        if hidden_size == 1024:
            n_head = 16
        ...
        for i in range(layers):
            blocks.append(AttnBlock(hidden_size, n_head))
